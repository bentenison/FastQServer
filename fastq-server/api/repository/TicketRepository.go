...
func (q *sqlTicketRepo) GetLastTicketNumber(ctx context.Context, service string) (models.TicketNumber, error) {
	row := q.db.QueryRowContext(ctx, `select ticket_number as number from ticket where date(created_at)=date(now()) and service = ? order by created_at desc limit 1;`, service)
