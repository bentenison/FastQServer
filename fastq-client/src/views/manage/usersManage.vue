        ...
        <v-card class="col-md-12 h-auto" flat>
          <div class="row d-flex" v-if="false">
            ...
            <v-col class="pl-0 col-md-2">
              <v-btn large text color="primary" class="mt-1 text-md-button">
          ...
          <v-col class="pl-0 col-md-2">
            <v-btn large text color="success" class="mt-1 text-md-button">
          ...
          <div class="row">
            ...
            <div class="col-md-12 mt-5 d-flex flex-wrap">
              <v-col cols="6" sm="6" class="">
                ...
                <v-btn color="primary" elevation="2" outlined :loading="false"
              ...
              <v-dialog v-model="show" max-width="500px">
                <v-card>
                  <v-card-title class="text-h5"
                    >You want to reset this user's login ?</v-card-title
                  >
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="primary darken-1" text @click="show = false"
                      >Cancel</v-btn
                    >
                    <v-btn color="red darken-1" text @click="resetLogins"
                      >OK</v-btn
                    >
                    <v-spacer></v-spacer>
                  </v-card-actions>
              ...
              <v-col cols="4" sm="4" class="">
                <h5 class="h4 grey--text">Reset User Logins</h5>
                <v-select
                ...
                <v-btn
                  ...
                  outlined
                  :loading="false"
                  ><v-icon left> mdi-upload </v-icon>Reset</v-btn
                >
              </v-col>
              <div class="col-md-12 d-flex flex-wrap">
                ...
                <v-col cols="12" sm="12" class="">
                  ...
                  <v-btn class="mr-3" color="primary" @click="addUser"
...
export default {
  ...
  methods: {
    getBranches() {
      axios
        .get(`/branch/getAllBranch/${this.$store.state.Auth.user.company_code}`)
        .then((res) => {
          //   console.log("res:::::", res);
    ...
    resetLogins() {
      ...
      //   console.log(">>>>>>>>>>>>>>>>>>>>>>.", this.selectedUser);
      axios
        .get(`/user/resetlogin//${this.selectedUser.ID}`)
        .then((res) => {
          //   console.log("res:::::", res);
          this.show = false;
          this.$toast.success("Login reset successfully.");
        ...
        .catch((err) => {
          console.log(err.response);
          this.show = false;
          this.$toast.error("error occured while resetting login!!!");
        });
    },
    addUser() {
