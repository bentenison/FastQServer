            ...
            <div class="row col-md-12 h-100">
                <v-card class="col-md-6 h-auto" flat>
                    ...
                    <div class="d-flex flex-column">
                        <p class="h6 grey--text">No Show Expiration <span class="caption">(seconds)</span></p>
                        <v-text-field type="number" v-model="ticketconf.no_show_exp" label="label" id="id"></v-text-field>
                    </div>
                    <div class="d-flex">
                    ...
                    <v-btn color="primary" class="" v-if="createOne" @click="addTicketConfig"><v-icon
                        ...
                        config</v-btn>
                    <v-btn color="primary" class="" v-if="!createOne"><v-icon left>mdi-update</v-icon>Update
                        config</v-btn>
                </v-card>
                <div class="col-md-5 p-3">
                    <h5 class="h6 grey--text">Image Header</h5>
                    <v-file-input label="Select a file to upload" filled prepend-icon="mdi-image-edit"
                        @change="handleFileChange" show-size accept="'image/jpeg,image/png'"></v-file-input>
                    <p class="caption">Maximum file size is 1 MB , only jpg/png files allowed Maximum dimension for width is
                        200</p>
                    <v-btn color="primary" elevation="2" outlined :loading="false" @click="uploadVideo"><v-icon left>
                            mdi-upload
                        </v-icon>Upload</v-btn>
                    ...
                    <div class="d-flex flex-column p-0">
                        ...
                        <v-text-field class="m-0 p-0" name="header text" v-model="ticketconf.header_text" label="header"
                            id="id"></v-text-field>
                    </div>
                    <!-- <div class="d-flex flex-column">
    ...
    methods: {
        ...
        addTicketConfig() {
            ...
            return new Promise((resolve, reject) => {
                ...
                axios.post(`/config/addticketconf`, this.ticketconf).then(res => {
                    console.log("res :::::>>>>>>>>>>", res);
        ...
        getTicketConf() {
            return new Promise((resolve, reject) => {
                ...
                // if (this.id) {
                this.$store.commit(types.MUTATE_LOADER_ON);
                axios.get(`/config/getticketconf/${this.$store.state.Auth.user.company_code}`).then(res => {
                    console.log("res :::::", res);
