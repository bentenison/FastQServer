        ...
        <div class="col-md-3 h-80" style="margin-top: 70px">
          <v-card class="h-100" flat>
            <v-card-title
              primary-title
              v-if="
                $store.state.Auth.counterUser.Firstname &&
                $store.state.Auth.activeCounter.CounterName &&
                $store.state.Auth.counterUser.Lastname
              "
            >
              {{ $store.state.Auth.activeCounter.CounterName.toUpperCase()
              }}<br />
            ...
            <div class="d-flex p-2">
              <v-card class="w-100" elevation="6">
                ...
                >
                  {{ $store.state.Auth.activeTicket.TicketName }}
                </h1>
              </v-card>
            </div>
            <hr />
                      ...
                      <Stopwatch
                        ...
                        :starter="
                          calculateTimestampDifference(
                            $store.state.Auth.activeTicket.StartedServingAt,
                  ...
                  <div class="col-md-12 d-flex justify-content-between">
                    ...
                    <div class="px-2">
                      <Stopwatch
                        ...
                        :starter="
                          calculateTimestampDifference(
                            $store.state.Auth.activeTicket.CreatedAt,
                            convertDateToCustomFormat(
                              convertDateToIndianTimestampFormat(new Date())
                            )
                          )
          ...
          <v-card class="h-100" flat>
            ...
            <div class="container">
              <div class="row text-white justify-content-between py-2">
                ...
                <v-card
                  class="col-md-2 warning accent-2 d-flex flex-column align-items-center text-white"
                  hover
                  ripple
                  :class="{ disabled: true }"
                >
                  <v-icon size="48px" color="white"> mdi-lock </v-icon>
                  <h6>Hold</h6>
                </v-card>
                <v-card
                  class="col-md-2 red accent-2 d-flex flex-column align-items-center text-white"
                  hover
                  ripple
                  @click.prevent="updateLastTicketStatus('NO_SHOW')"
                >
                  <v-icon size="48px" color="white"> mdi-close-octagon </v-icon>
                  <h6>No Show</h6>
                </v-card>
                <v-card
                  class="col-md-2 green accent-4 d-flex flex-column align-items-center text-white"
                  hover
                  ripple
                  @click.prevent="updateLastTicketStatus('FINISHED')"
                >
                  <v-icon size="48px" color="white"> mdi-check </v-icon>
                  <h6>Close</h6>
                </v-card>
              </div>
            </div>
            <div class="container mt-3 w-100">
              <div class="row">
                ...
                <div
                  class="col-md-9 d-flex flex-wrap gap-2 justify-center align-center"
                  v-if="allconf.ds_conf && allconf.ds_conf.confirmation_delay"
                >
                  ...
                  <v-col cols="12" sm="12" class="m-0 p-0 text-center">
                    <v-btn
                      color="primary text-white green accent-4"
              ...
              <v-card class="col-md-4 h-100" flat>
                <div class="row">
                  ...
                  <div class="p-3">
                    ...
                    <v-card
                      class="d-flex p-2 align-items-center justify-content-between mb-2"
                      elevation="5"
                      outlined
                      shaped
  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (res.data && res.data !== "NO_RESULT") {
            ...
            res.data.StartedServingAt = this.convertDateToCustomFormat(
              this.convertDateToIndianTimestampFormat(new Date())
            );
    ...
    getlastongoingticket() {
      return new Promise((resolve, reject) => {
        axios
          .get(
            `/ticket/getlaststarted/${this.$store.state.Auth.counterUser.ID}`
          )
          .then((res) => {
    ...
    updateLastTicketStatus(status) {
      ...
      axios
        .post("/ticket/updateticketstatus", payload)
        .then((res) => {
          // console.log("res:::::", res);
          ...
          if (
            (status === "NO_SHOW" || status === "FINISHED") &&
            this.$store.state.Auth.activeTicket &&
            this.$store.state.Auth.activeTicket !== "NO_RESULT"
          ) {
    ...
    updatestartTime() {
      ...
      axios
        .post("/ticket/updatetickettime", payload)
        .then((res) => {
          // console.log("res:::::", res);
          // this.noShow = res.data
          // this.$toast.success("branch added successfully.")
    ...
    updateEndTime() {
      ...
      axios
        .post("/ticket/updateendtime", payload)
        .then((res) => {
          // console.log("res:::::", res);
          // this.noShow = res.data
          // this.$toast.success("branch added successfully.")
    ...
    getnoshow() {
      axios
        .get(`/ticket/getnoshow/${this.$store.state.Auth.activeCounter.ID}`)
        .then((res) => {
          // console.log("res:::::", res);
          if (!res.data) {
            this.noShow = [];
    ...
    getcompleted() {
      axios
        .get(`/ticket/getfinished/${this.$store.state.Auth.activeCounter.ID}`)
        .then((res) => {
          // console.log("res:::::", res);
          if (!res.data) {
            this.finished = [];
