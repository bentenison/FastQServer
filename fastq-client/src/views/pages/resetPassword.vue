        ...
        <v-container fill-height>
            <v-layout row align-center justify-center>
                <v-card flat light max-width="400px" align-center>
                    ...
                    <v-divider></v-divider>
                    <v-card-text>
                        <p>Enter a new password and reset:</p>
                        <v-form>
                            <v-text-field outline label="Password" :rules="nameRules" type="text"
                                v-model="username"></v-text-field>
                            <v-text-field outline label="Confirm Password" :rules="passwordRules" type="text"
                                v-model="username"></v-text-field>
                        </v-form>
                    </v-card-text>
                    <v-divider></v-divider>
                    <v-card-actions :class="{ 'pa-3': $vuetify.breakpoint.smAndUp }">
                        <v-btn color="primary" class="" text :large="$vuetify.breakpoint.smAndUp">
                            <v-icon left color="primary">mdi-restore</v-icon>
                            Reset Password
                        </v-btn>
                    </v-card-actions>
                </v-card>
