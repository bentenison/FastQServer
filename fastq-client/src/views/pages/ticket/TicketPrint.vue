<template>
  <v-app>
    <v-app-bar flat rounded app>
      ...
      <v-img
        ...
        :src="
          ipAddress && ipAddress !== 'localhost'
            ? `https://${ipAddress}:443/uploaded/logo.png`
      ...
      >
        {{ this.$store.state.Auth.user.company.toUpperCase() }}
      </h3>
      <v-spacer></v-spacer>
      <div class="d-flex justify-content-around m-auto">
        <h3 class="time mr-3">{{ time }}</h3>
        <h3 class="time mr-3">{{ date }}</h3>
        ...
        <!-- <v-chip v-if="true" class="ma-2" color="red" text-color="white">
                    DISCONNECTED
                </v-chip> -->
        <v-chip class="ma-2" v-if="device" color="#0089A3" outlined>
          <v-icon left> mdi-server-plus </v-icon>
          Connected
        </v-chip>
        <v-tooltip bottom v-if="bluetooth">
          ...
          <span>Bluetooth Printer Status</span>
        </v-tooltip>

        <!-- <v-chip v-if="true" class="ma-2" color="success" text-color="white">
                    CONNECTED
                </v-chip> -->
        <v-tooltip bottom v-if="bluetooth">
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              fab
              small
              :color="device ? '#0089A3' : '#AD1457'"
              class="mx-2"
              @click.prevent="connectBluetoothPrinter"
              v-bind="attrs"
              v-on="on"
            >
      ...
      >
        ...
        <div class="row col-md-12 align-content-center justify-content-center">
          ...
          <v-card
            v-for="n in services"
            :key="n.id"
            class="mb-3 col-md-4 mr-5 text-center"
            @click="addTicket(n)"
            shaped
            hover
            color="primary"
