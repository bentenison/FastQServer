        ...
        <div class="row h-100">
            <div class="col-md-12 h-100">
                <v-card class="row h-100">
                    <DataTables :headers="headers" :items="tableData" />
                    ...
                    <v-card class="h-70" flat>
                        <div class="col-md-6 p-3">
                            ...
                            <v-file-input label="Select a file to upload" filled prepend-icon="mdi-video"
                                v-model="selectedFile" @change="handleFileChange" :loader-height="3"></v-file-input>
                            <p class="caption">Maximum file size is 100 MB , only mp4/MP4 files allowed</p>
                ...
                axios.get(`/config/getallvideo/${this.$store.state.Auth.user.company_code}`).then(res => {
                    ...
                    if (res.data){
                        res.data.forEach(element => {
                            ...
                            data.modified_at = moment(element.modified_at).format('MMMM DD, YYYY HH:mm:ss');
                            data.created_at = moment(element.created_at).format('MMMM DD, YYYY HH:mm:ss');
                            data.id = element.id
